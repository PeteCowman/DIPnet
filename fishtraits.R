...
spp = read.csv("DIPnet_species_fixed.csv", header = T)
spp = str_c(spp$genus, spp$species, sep = " ")
spp = gsub("  ", " ", spp)
# fix Acanthaster
spp[grep("Acanthaster", spp)] = "Acanthaster planci"
spp = unique(spp)
genera = unlist(stri_split_fixed(spp, " ",  n=1, tokens_only=TRUE))
dip.fish = spp[genera %in% all.gen]
dip.invert = spp[genera %ni% all.gen]
##########
# check synonyms
...
#########
# get info tables
# dip.fish.info = species_info(dip.fish[1:5]) # takes a long time to run...
...
dip.fish.info = read.csv("FishbaseTraits.csv")
fb.depth = dip.fish.info[,c(2:4, 99, 26:31)]
...
selkoe_all = selkoe[selkoe$Species %in% spp,]
...
write.csv(DIP_mb_fish, "~//Google Drive//DIPnet_work//DIPnet_WG4//PCM for Genetic Diversity//Life history Traits//Processed_data/Maria_fishtraits_DIP.csv")
##########################
...
DIP_luiz_traits = luiz_traits[luiz_traits$Genus_species %in% spp,]
length(DIP_luiz_traits$Genus_species)
write.csv(DIP_luiz_traits, "~//Google Drive//DIPnet_work//DIPnet_WG4//PCM for Genetic Diversity//Life history Traits//Processed_data/luiz_dip_traits.csv")
